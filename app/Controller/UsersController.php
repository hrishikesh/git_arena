...
class UsersController extends AppController {
    ...
    public function dashboard() {
        ...
        if ($this->Auth->login($userInfo['User'])) {
            $this->Session->write('User.avatar_url', $user['avatar_url']);
            $this->redirect(array(
                                 'controller' => 'users',
                                 'action'     => 'home'
                            ));
    ...
    public function home() {
        ...
        $repos     = $this->GitHubApi->getRepoList();
        $avatarUrl = $this->Session->read('User.avatar_url');
        $commits   = $this->GitHubApi->getCommitsForRepo($repos[0]['owner']['login'], $repos[0]['name']);
        $contributors = $this->GitHubApi->getRepoContributors($repos[0]['owner']['login'], $repos[0]['name']);
        $this->set(compact('repos', 'commits', 'userName', 'avatarUrl','contributors'));
    }
